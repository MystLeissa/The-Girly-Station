...
/obj/item/hand_tele/attack_self(mob/user)
	...
	if(!current_location || current_area.noteleport || current_area.noteleexit || is_away_level(current_location.z) || !isturf(user.loc))//If turf was not found or they're on z level 2 or >7 which does not currently exist. or if user is not located on a turf
